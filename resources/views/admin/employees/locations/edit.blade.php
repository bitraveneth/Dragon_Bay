...
<div class="max-w-4xl mx-auto space-y-8">
            ...
            <div class="flex items-start gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                        Update timestamp, coordinates, or notes for this location log
                    </p>
                    <div class="mt-3 flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <span>Created {{ $log->created_at?->diffForHumans() }}</span>
                        <span class="inline-flex h-1 w-1 rounded-full bg-gray-300 dark:bg-gray-700"></span>
                        <span>Last updated {{ $log->updated_at?->diffForHumans() }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="rounded-3xl border border-gray-200 bg-white shadow-xl dark:border-gray-800 dark:bg-gray-900 overflow-hidden">
        ...
        <div class="border-b border-gray-100 bg-gradient-to-r from-brand-50 to-white px-8 py-6 dark:border-gray-800 dark:from-brand-950/30 dark:to-gray-900">
            ...
            <div class="mt-4 flex items-start gap-3 rounded-xl bg-brand-100/50 p-4 text-brand-800 dark:bg-brand-900/30 dark:text-brand-300">
                ...
                <div class="text-sm flex-1">
                    <span class="font-semibold">Current coordinates:</span>
                    <span class="ml-2 font-mono">{{ number_format($log->latitude, 6) }}, {{ number_format($log->longitude, 6) }}</span>
                    <a href="https://www.google.com/maps?q={{ $log->latitude }},{{ $log->longitude }}"
                       target="_blank"
                       rel="noopener"
                       class="ml-3 inline-flex items-center gap-1 text-xs font-medium text-brand-700 hover:text-brand-800 dark:text-brand-400 dark:hover:text-brand-300">
                        View on map
        ...
        <div class="p-8">
            <form action="{{ route('admin.employees.locations.update', [$employee, $log]) }}" method="POST">
                @csrf
                @method('PATCH')
    ...
    <div class="rounded-2xl border border-error-200 bg-white shadow-sm dark:border-error-800/30 dark:bg-gray-900">
        ...
        <div class="p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-900 dark:text-white">Delete this location log</p>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                        Once deleted, this location record cannot be recovered. This action cannot be undone.
                    </p>
                </div>
                <form action="{{ route('admin.employees.locations.destroy', [$employee, $log]) }}" 
                      method="POST" 
                      onsubmit="return confirm('Are you sure you want to delete this location log from {{ $log->logged_at?->format('Y-m-d H:i') }}? This action cannot be undone.');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" 
