...
<div class="max-w-4xl mx-auto space-y-8">
        ...
        <div>
            <div class="flex items-start gap-4">
                ...
                <div>
                    ...
                    <div class="mt-3 flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                        <span>Assigned {{ $item->created_at?->diffForHumans() }}</span>
                        <span class="inline-flex h-1 w-1 rounded-full bg-gray-300 dark:bg-gray-700"></span>
                        <span>Last updated {{ $item->updated_at?->diffForHumans() }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="rounded-3xl border border-gray-200 bg-white shadow-xl dark:border-gray-800 dark:bg-gray-900 overflow-hidden">
        ...
        <div class="border-b border-gray-100 bg-gradient-to-r from-brand-50 to-white px-8 py-6 dark:border-gray-800 dark:from-brand-950/30 dark:to-gray-900">
            ...
            <div class="mt-4 flex items-start gap-3 rounded-xl bg-brand-100/50 p-4 text-brand-800 dark:bg-brand-900/30 dark:text-brand-300">
                ...
                <div class="text-sm flex-1">
                    ...
                    <span class="ml-2 font-medium">{{ $item->product_name }}</span>
                    @if($item->model)
                        <span class="ml-2 text-xs text-gray-600 dark:text-gray-400">({{ $item->model }})</span>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="p-8">
            <form action="{{ route('admin.employees.equipment.update', [$employee, $item]) }}" method="POST">
                @csrf
                @method('PATCH')
    ...
    <div class="rounded-2xl border border-error-200 bg-white shadow-sm dark:border-error-800/30 dark:bg-gray-900">
        ...
        <div class="p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-900 dark:text-white">Remove this equipment assignment</p>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                        Once removed, this equipment record cannot be recovered. This action cannot be undone.
                    </p>
                </div>
                <form action="{{ route('admin.employees.equipment.destroy', [$employee, $item]) }}" 
                      method="POST" 
                      onsubmit="return confirm('Are you sure you want to remove {{ $item->product_name }} from {{ $employee->name }}? This action cannot be undone.');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" 
