...
<div class="space-y-8">
                ...
                <div class="relative">
                    ...
                    <div class="relative flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-brand-500 to-brand-600 text-white shadow-lg">
                        <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2zM8 7h8M8 11h6M8 15h4" />
                        </svg>
                    </div>
                </div>
                <div>
    ...
    @if($rows->isNotEmpty())
        @php
            $productTotals = $rows->groupBy(function ($row) {
                return $row->product->id ?? 0;
            })->map(function ($group) {
                return $group->sum('quantity');
            });
            
            $warehouseTotals = $rows->groupBy(function ($row) {
                return $row->warehouse->id ?? 0;
            })->map(function ($group) {
                return $group->sum('quantity');
            });
            
            $totalQuantity = $rows->sum('quantity');
            $uniqueMaterials = $rows->pluck('product.id')->unique()->count();
            $uniqueWarehouses = $rows->pluck('warehouse.id')->unique()->count();
        @endphp
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <div class="relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition-all hover:shadow-md dark:border-gray-800 dark:bg-gray-900">
                ...
                <div class="relative">
                    <div class="flex items-center justify-between">
                        <span class="text-sm font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Materials</span>
                    ...
                    <p class="mt-3 text-3xl font-bold text-gray-900 dark:text-white">{{ $uniqueMaterials }}</p>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Unique raw materials</p>
                </div>
            </div>
            ...
            <div class="relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition-all hover:shadow-md dark:border-gray-800 dark:bg-gray-900">
                ...
                <div class="relative">
                    ...
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Storage locations</p>
                </div>
            </div>
            ...
            <div class="relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition-all hover:shadow-md dark:border-gray-800 dark:bg-gray-900">
                ...
                <div class="relative">
                    <div class="flex items-center justify-between">
                        <span class="text-sm font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Avg per Material</span>
                    ...
                    <p class="mt-3 text-3xl font-bold text-gray-900 dark:text-white">{{ number_format($uniqueMaterials > 0 ? $totalQuantity / $uniqueMaterials : 0, 2) }}</p>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Average quantity per material</p>
                </div>
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-gray-50 dark:bg-gray-800/50">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-400">Material</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-400">UOM</th>
                            ...
                            <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-400">Quantity</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                        @foreach($rows as $row)
                            @php
                                $pid = $row->product->id ?? null;
                                $isFirstForProduct = $loop->first || ($pid && $pid !== ($rows[$loop->index - 1]->product->id ?? null));
                            @endphp
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                <td class="px-4 py-3">
                                    <div>
                                        ...
                                        @if($pid && isset($productTotals[$pid]) && $isFirstForProduct)
                                            <div class="mt-2 flex items-center gap-1.5 text-xs">
                                                <span class="text-gray-500 dark:text-gray-400">Total across all warehouses:</span>
                                                <span class="font-semibold text-brand-600 dark:text-brand-400">{{ number_format($productTotals[$pid], 2) }}</span>
                                                <span class="text-gray-500 dark:text-gray-400">{{ $row->product->uom ?? '' }}</span>
                                            </div>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="inline-flex items-center gap-2">
                                        ...
                                        @if($row->product->uom)
                                            <span class="text-xs text-gray-500 dark:text-gray-400">{{ $row->product->uom }}</span>
                                        @endif
                                    </div>
                                </td>
                            </tr>
        ...
        <div class="relative overflow-hidden rounded-3xl border border-gray-200 bg-white/50 backdrop-blur-sm p-16 text-center shadow-sm dark:border-gray-800 dark:bg-gray-900/50">
            ...
            <div class="relative">
                <div class="mx-auto flex h-28 w-28 items-center justify-center rounded-full bg-gradient-to-br from-gray-100 to-gray-50 dark:from-gray-800 dark:to-gray-700">
                    <div class="flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-gray-400 to-gray-500 text-white shadow-lg">
                        <svg class="h-10 w-10" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                        </svg>
                    </div>
                </div>
                <h2 class="mt-8 text-2xl font-bold text-gray-900 dark:text-white">No Material Stock</h2>
                <p class="mt-3 text-base text-gray-500 dark:text-gray-400 max-w-md mx-auto">
                    No raw material stock has been recorded yet. Stock will appear here when materials are received or transferred to warehouses.
                </p>
