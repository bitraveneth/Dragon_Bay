...
<div class="dashboard-shell">
    <section class="panel rounded-xl border border-gray-200 bg-white p-6 shadow-theme-sm dark:border-gray-800 dark:bg-gray-900">
        <header class="panel-header mb-6 flex items-start justify-between">
            <div>
                <h1 class="text-title-sm font-semibold text-gray-900 dark:text-white">Edit expense</h1>
                <p class="text-theme-sm text-gray-500 dark:text-gray-400">Update amount, category, or status.</p>
            </div>
...
@push('scripts')
<script>
    // Optional: Add confirmation for cancel if form is dirty
    document.addEventListener('DOMContentLoaded', function() {
        const form = document.querySelector('form');
        const cancelBtn = document.querySelector('a[href="{{ route('admin.expenses.index') }}"]');
        let formDirty = false;
