...
<div class="dashboard-shell">
    <section class="panel rounded-xl border border-gray-200 bg-white p-6 shadow-theme-sm dark:border-gray-800 dark:bg-gray-900">
        <header class="panel-header mb-6 flex items-start justify-between">
            <div>
                <h1 class="text-title-sm font-semibold text-gray-900 dark:text-white">Add salary distribution</h1>
                <p class="text-theme-sm text-gray-500 dark:text-gray-400">Record salary and allowances for one employee and period.</p>
            </div>
        ...
        <form action="{{ route('admin.salary-distributions.store') }}" method="POST" 
              class="space-y-6" enctype="multipart/form-data" id="salary-distribution-form">
            @csrf
            @include('admin.finance.salary_distributions.partials.form', ['distribution' => $distribution ?? null])
            ...
            <div class="form-actions flex items-center justify-end gap-3 border-t border-gray-200 pt-6 dark:border-gray-800">
                ...
                <button type="submit" 
                    ...
                    <span class="inline-flex items-center gap-2">
                        ...
                        <span class="submit-text">Save distribution</span>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.addEventListener('pageshow', function(e) {
            if (e.persisted) {
                ...
                submitText.textContent = 'Save distribution';
                submitIcon.classList.remove('hidden');
                submitSpinner.classList.add('hidden');
                
                // Reset form dirty detection
                setTimeout(() => {
                    originalFormData = new FormData(form);
                    formDirty = false;
                }, 100);
